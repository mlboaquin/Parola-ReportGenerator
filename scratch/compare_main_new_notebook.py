...
for name in suspects:
    ...
    if m_body is None and n_body is None:
        ...
        report.append(f"Function {name} ONLY in new_notebook.txt.")
    elif n_body is None:
        ...
        if m_norm_ws != n_norm_ws:
            diff = list(difflib.unified_diff(
                ...
                tofile='new_notebook.txt: ' + name,
                lineterm=''
            ))
            report.append(f"Diff for {name}:\n" + '\n'.join(diff[:20]) + ("\n...truncated" if len(diff) > 20 else ""))
        else:
            report.append(f"Function {name} is identical (modulo self).")

with open('scratch/new_diff_report.txt', 'w', encoding='utf-8') as outf:
    outf.write('\n\n'.join(report))
